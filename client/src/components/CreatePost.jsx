...
axios.get(`${import.meta.env.VITE_BASE_URL}/api/createposts`)
  ...
  const handleCreateOrEditPost = async (e) => {
    ...
    try {
      ...
      if (editingPostId) {
        // UPDATED: Changed endpoint from /api/events to /api/createposts
        res = await axios.put(`${import.meta.env.VITE_BASE_URL}/api/createposts/${editingPostId}`, formData, {
...
, formData, {
  ...
  const handleDelete = () => {
    // UPDATED: Changed endpoint from /api/events to /api/createposts
    axios
      .delete(`${import.meta.env.VITE_BASE_URL}/api/createposts/${confirmDeleteId}`)
      .then(() => {
